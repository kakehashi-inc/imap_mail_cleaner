...
**絶対に実行してはいけないこと:**
- venv環境の作成または変更（venvディレクトリは既に作成済み）
- pipやパッケージマネージャーを使用したパッケージのインストール
- パッケージインストールコマンドの実行
- `install_python_packages` ツールの使用
- `configure_python_environment` ツールのインストール目的での使用
- 環境情報の読み取りはデバッグに必要な場合のみ許可

**既存のvenv環境は事前設定済みであり、変更してはならない。**
